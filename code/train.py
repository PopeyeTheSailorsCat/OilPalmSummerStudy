...
def rec(input1):
    x = Conv2D(int(float(input1.get_shape().as_list()[-1]) / float(4)), (1, 1), activation="relu", padding="same")(
        input1)
...
def train(args):
    ...
    H = model.fit_generator(generator=generateData(BS, train_set), steps_per_epoch=train_numb // BS, epochs=EPOCHS,
                            verbose=1, validation_data=generateValidData(BS, val_set),
                            validation_steps=valid_numb // BS, callbacks=callable, max_q_size=1)
